...
function getUnixDate(inputString) {
  return moment(inputString, 'E YYYY/MM/DD HH:mm:ss').unix();
}
...
Parser.prototype.parseLine = function parseLine(line, cb) {
  ...
  if (startsLine.test(line)) {
    lease.startDate = getUnixDate(startsLine.exec(line)[1]);
    return;
  }

  if (endsLine.test(line)) {
    lease.endDate = getUnixDate(endsLine.exec(line)[1]);
    return;
  }

  if (tstpLine.test(line)) {
    lease.tstp = getUnixDate(tstpLine.exec(line)[1]);
    return;
  }

  if (tsfpLine.test(line)) {
    lease.tsfp = getUnixDate(tsfpLine.exec(line)[1]);
    return;
  }

  if (atsfpLine.test(line)) {
    lease.atsfp = getUnixDate(atsfpLine.exec(line)[1]);
    return;
  }

  if (clttLine.test(line)) {
    lease.cltt = getUnixDate(clttLine.exec(line)[1]);
    return;
  }
