...
function getIsoDate(inputString) {
  return moment(inputString, 'E YYYY/MM/DD HH:mm:ss').toISOString();
}
...
Parser.prototype.parseLine = function parseLine(line, cb) {
  ...
  if (startsLine.test(line)) {
    lease.startDate = getIsoDate(startsLine.exec(line)[1]);
    return;
  }

  if (endsLine.test(line)) {
    lease.endDate = getIsoDate(endsLine.exec(line)[1]);
    return;
  }

  if (tstpLine.test(line)) {
    lease.tstp = getIsoDate(tstpLine.exec(line)[1]);
    return;
  }

  if (tsfpLine.test(line)) {
    lease.tsfp = getIsoDate(tsfpLine.exec(line)[1]);
    return;
  }

  if (atsfpLine.test(line)) {
    lease.atsfp = getIsoDate(atsfpLine.exec(line)[1]);
    return;
  }

  if (clttLine.test(line)) {
    lease.cltt = getIsoDate(clttLine.exec(line)[1]);
    return;
  }
